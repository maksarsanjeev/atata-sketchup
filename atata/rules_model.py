...
def r_geometry_total(f: SkpFacts) -> Finding | None:
    ...
    return Finding(
        ...
        summary=(
            ...
            f"даёт сорок наборов геометрии.\n\n"
            f"В корне модели {model.root_faces:,} граней и {model.root_edges:,} рёбер, "
            f"остальное приходит из компонентов. Для интерьерной сцены разумный "
            f"потолок — сотни тысяч граней; выше начинает проседать вьюпорт."
        ).replace(",", " "),
        layer="model",
...
def r_heavy_components(f: SkpFacts) -> Finding | None:
    ...
    share = sum(d.total_faces for d in heavy)
    return Finding(
        id="geometry.heavy_components",
        severity="high",
        category="геометрия",
        title=f"{len(heavy)} компонентов дают {share:,} граней".replace(",", " "),
        summary=(
