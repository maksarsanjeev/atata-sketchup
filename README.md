...
Цель правок — **устойчивость**, а не вес файла: снять нагрузку на память и
видеопамять, убрать явный мусор, починить внутренние ошибки. Файл худеет
побочно.

| Исправление | Что делает |
|---|---|
| `fix_errors` | штатная починка модели — то же, что «Fix Problems» в SketchUp |
| `erase_loose_edges` | рёбра, не принадлежащие ни одной грани |
| `normalize_textures` | текстуры к степени двойки — снимает перерасход видеопамяти |
| `purge_unused` | определения компонентов, не вставленные в модель |

Все четыре идут за один сеанс: модель открывается и сохраняется по разу.
Порядок фиксирован — чинить, убирать мусор, править текстуры, и только потом
выбрасывать неиспользуемое.

Замер на рабочем файле, все четыре сразу (146 с локально, 280 с под Wine):

```
fix_errors          прогнан; ошибок на этом файле не нашлось
erase_loose_edges   450 429 рёбер убрано, 731 контейнер защищён
normalize_textures  328 из 474; больше 2048px не осталось,
                    не-степеней-двойки 19 вместо 275
purge_unused        2247 определений, 3820 -> 1573

348.6 -> 167.1 МБ, файл открывается читателем SketchUp
```

Геометрия сцены не изменилась: те же 4 545 548 граней, 6 сцен, 147 тегов.

**Про висячие рёбра.** Контейнеры, где нет ни одной грани, не трогаются:
компонент из одних линий — это 2D-подложка, план или разметка, то есть
содержимое, а не мусор. На этом файле так уцелело 731 контейнер.

**Про текстуры.** `SUTextureGetDimensions` и `SUTextureCreateFromFile`
пользуются обратными соглашениями о масштабе — `0.1` на чтении соответствует
`10.0` на записи. Без обратной величины у всех текстур меняется размер плитки
в модели. Инвариант проверяется после каждой замены и попадает в отчёт полем
`texture_scale_kept`.
...
- [x] Штатная починка модели, чистка висячих рёбер, текстуры к степени двойки
- [ ] Убрать из отчёта находки, которые про вес на диске, а не про устойчивость
- [ ] Вырожденные грани нулевой площади
- [ ] Атрибуты плагинов: V-Ray-словари на каждом материале
