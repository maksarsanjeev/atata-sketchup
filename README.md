...
в архиве SDK лежат `SketchUpAPI.dll` / `.framework` и ни одного `.so`. Это значит, что
внутри Linux-контейнера слой геометрии не работает: сервис поднимается без него и
честно пишет в отчёте, что цифры по `model.dat` — косвенная оценка.

Чтобы включить разбор геометрии, нужен воркер на Windows или macOS.

SDK **не входит в репозиторий**: он распространяется под Trimble Developer Terms,
принимать которые должен лицензиат. Скачайте его сами:

1. Возьмите SDK на [developer.sketchup.com](https://developer.sketchup.com/) →
   Desktop SDK, версии не ниже той, в которой сохранены ваши файлы
...
Обвязка — чистый `ctypes` над плоским C-API, компилировать ничего не нужно.
...
Поле `sdk.available` покажет состояние, `sdk.reason` — причину, если нет.

> **Проверьте лицензию под свою задачу.** Trimble Developer Terms отдельно
> регулируют серверное и коммерческое использование SDK. Для домашнего стенда
> это не вопрос, для публичного сервиса — вопрос к юристам.
...
- Слой геометрии требует Windows или macOS: под Linux SDK не существует.
  В Linux-контейнере сервис работает без него, на косвенных оценках.
...
- [ ] Прогнать обвязку на настоящем SDK — сигнатуры объявлены по документации,
      на живой библиотеке ещё не проверялись
- [ ] Вынести SDK-разбор в отдельный воркер на Windows/macOS с очередью
...
Логотип — пародийный. SketchUp и знак SketchUp — торговые марки Trimble Inc.;
проект с Trimble никак не связан и ими не одобрен.
Использование SketchUp SDK на сервере и в коммерческом сервисе требует отдельной
проверки Trimble Developer Terms.
